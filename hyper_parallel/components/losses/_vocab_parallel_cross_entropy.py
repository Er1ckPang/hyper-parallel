...
class _CrossEntropyOptions:
    """Reduction and sharding options for the forward pass."""
...
def _cross_entropy_forward(
    ...
    target: Tensor,
    weight: Optional[Tensor],
    options: _CrossEntropyOptions,
) -> Tensor:
    """Compute the local loss, apply its reduction, and save backward state."""
    vocab_start = _compute_vocab_start(
        options.vocab_size,
        options.mesh.size(options.mesh_dim),
        options.mesh.get_local_rank(options.mesh_dim),
    )
    vocab_end = vocab_start + input_local.shape[-1]
    log_probs_local = distributed_log_softmax(
        input_local, dim=-1, mesh=options.mesh, mesh_dim=options.mesh_dim
    )
    nll_result = distributed_nll_loss_forward(
        log_probs_local, target, weight, options.ignore_index,
        options.reduction, vocab_start, vocab_end,
    )
    loss, total_weight = nll_result[:2]
    if options.reduction in ("mean", "sum"):
        group = options.mesh.get_group(options.mesh_dim)
        loss = platform.differentiable_all_reduce(loss, op="sum", group=group)
        if options.reduction == "mean":
            total_weight = platform.differentiable_all_reduce(total_weight, op="sum", group=group)
    if options.reduction != "mean":
    ...
    _save_cross_entropy_context(
        ...
        _CrossEntropyContext(
            log_probs_local=log_probs_local,
            target=target,
            weight=weight,
            total_weight=total_weight,
            target_mask=nll_result[2],
            vocab_start_tensor=nll_result[3],
...
class DistributedCrossEntropyFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        """Forward pass."""
        return _cross_entropy_forward(
            ctx,
            input_local,
            target,
            weight,
            _CrossEntropyOptions(ignore_index, reduction, vocab_size, mesh, mesh_dim),
        )
